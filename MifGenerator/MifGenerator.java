...
public class MifGenerator {
    ...
    public void Writing(String file) {
        try {
            ...
            writer.println("DATA_RADIX = HEX;");                                   // edited to hexadecimal
            writer.println("CONTENT");
            writer.println("BEGIN");
            for (int i = 0; i < words.length; i+=4){
				int val0 = Integer.parseInt(words[i]);
				int val1 = Integer.parseInt(words[i+1]);
				int val2 = Integer.parseInt(words[i+2]);
				int val3 = Integer.parseInt(words[i+3]);

				String byte0 = Integer.toHexString(val0).toUpperCase();
				String byte1 = Integer.toHexString(val1).toUpperCase();
				String byte2 = Integer.toHexString(val2).toUpperCase();
				String byte3 = Integer.toHexString(val3).toUpperCase();

                writer.println(""+ counter + " : " + byte0 + byte1+ byte2 +byte3 + ";");   // big endian
                counter ++;
            }
